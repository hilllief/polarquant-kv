...
from polarquant_kv_cuda.flash_attention_native import (
    flash_compressed_attention_v3, flash_compressed_attention_v5,
)
...
print(f"\n{'seq':>6}|{'SDPA':>8}|{'V3':>8}|{'V5':>8}|{'V5/SDPA':>8}|{'Cos':>6}")
print(f"{'-'*6}|{'-'*8}|{'-'*8}|{'-'*8}|{'-'*8}|{'-'*6}")
...
for S in [512, 2048, 4096, 8192, 16384]:
    ...
    out_v5 = flash_compressed_attention_v5(Q, ck, cv, R)
    cos = torch.nn.functional.cosine_similarity(
        out_std.flatten().float(), out_v5.flatten().float(), dim=0).item()
    print(f"{S:>6}|{sdpa:>6.3f}ms|{v3:>6.3f}ms|{v5:>6.3f}ms|{sdpa/v5:>6.2f}x|{cos:>5.3f}")
