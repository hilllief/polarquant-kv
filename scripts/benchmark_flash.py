"""压缩版 Flash Attention benchmark — V1 vs V3 (warp shuffle)。"""
import torch, math
...
from polarquant_kv_cuda.flash_attention_native import (
    flash_compressed_attention, flash_compressed_attention_v3,
)
...
Hq = 32
print(f"\n{'seq':>6} | {'SDPA':>8} | {'V1':>8} | {'V3warp':>8} | {'V3 Speed':>9} | {'Cos':>6}")
print(f"{'-'*6} | {'-'*8} | {'-'*8} | {'-'*8} | {'-'*9} | {'-'*6}")
...
for S in [512, 2048, 4096, 8192, 16384]:
    ...
    v1_lat = measure(lambda: flash_compressed_attention(Q, ck, cv, R))
    v3_lat = measure(lambda: flash_compressed_attention_v3(Q, ck, cv, R))

    out_std = torch.nn.functional.scaled_dot_product_attention(Q.float(), K.float(), V.float()).half()
    out_v3 = flash_compressed_attention_v3(Q, ck, cv, R)
    cos = torch.nn.functional.cosine_similarity(
        out_std.flatten().float(), out_v3.flatten().float(), dim=0).item()

    print(f"{S:>6} | {sdpa_lat:>6.3f}ms | {v1_lat:>6.3f}ms | {v3_lat:>6.3f}ms | {sdpa_lat/v3_lat:>7.2f}x | {cos:>5.3f}")
